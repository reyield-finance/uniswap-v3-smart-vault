...
interface WETH9Fixture {
  weth9Fixture: MockWETH9;
}
...
interface RegistryAddressHolderFixture {
  registryAddressHolderFixture: RegistryAddressHolder;
}
...
export function hexToInt256(largeNumber: bigint): bigint {
  const size = 256n;
  const sign = 1n << (size - 1n);

  return (largeNumber & (sign - 1n)) - (largeNumber & sign);
}
...
export function parseEventData(eventData: string): string[] {
  eventData = eventData.replace("0x", "");
  const result: string[] = [];
  for (let i = 0; i < eventData.length; i += 64) {
    result.push(eventData.slice(i, i + 64));
  }
  return result;
}
...
export async function weth9Fixture(): Promise<WETH9Fixture> {
  const weth9Factory = await ethers.getContractFactory("MockWETH9");
  const weth9Fixture: MockWETH9 = (await weth9Factory.deploy()) as MockWETH9;
  return { weth9Fixture };
}

export async function RegistryAddressHolderFixture(registry: string): Promise<RegistryAddressHolderFixture> {
  const registryAddressHolderFactory = await ethers.getContractFactory("RegistryAddressHolder");
  const registryAddressHolderFixture: RegistryAddressHolder = (await registryAddressHolderFactory.deploy(
    registry,
  )) as RegistryAddressHolder;
  return { registryAddressHolderFixture };
}
...
export async function RegistryFixture(
  ...
  twapDuration: number,
  usdValueTokenAddress: string,
  weth9: string,
): Promise<RegistryFixture> {
  const registryFactory = await ethers.getContractFactory("Registry");
  const registryFixture: Registry = (await registryFactory.deploy(
    ownerAddress,
    serviceFeeRecipient,
    maxTwapDeviation,
    twapDuration,
...
export async function poolFixture(
  ...
  const pool = new ethers.Contract(receipt.events[0].args.pool, UniswapV3Pooljson["abi"], signers[0]) as IUniswapV3Pool;
...
export async function depositWETH9Amount(weth9: MockWETH9, amount?: string) {
  const signers = await ethers.getSigners();
  await weth9.connect(signers[0]).deposit({ value: ethers.utils.parseEther(amount || "1000") });
  await weth9.connect(signers[1]).deposit({ value: ethers.utils.parseEther(amount || "1000") });
  await weth9.connect(signers[2]).deposit({ value: ethers.utils.parseEther(amount || "1000") });
  await weth9.connect(signers[3]).deposit({ value: ethers.utils.parseEther(amount || "1000") });
}

// get function selectors from ABI
export async function getSelectors(contract: Contract) {
...
export async function mintForkedTokens(MockTokens: any[], users: any[], amount: any) {
  for (let j = 0; j < MockTokens.length; j++) {
    for (let i = 0; i < users.length; i++) {
      ...
      const probedSlot = ethers.utils.keccak256(encode(["address", "uint"], [users[i].address, slot]));
      const value = encode(["uint"], [ethers.utils.parseEther(amount.toString())]);
...
export async function getPositionManager(PositionManagerFactory: any, user: Signer): Promise<Contract> {
  ...
  const PositionManager = await ethers.getContractAt(PositionManagerjson["abi"], managers[managers.length - 1]);
...
export async function getMainnetContracts() {
  ...
  contracts.PoolDaiUsdc100 = (await ethers.getContractAt(
    UniswapV3Pooljson.abi,
    "0x5777d92f208679DB4b9778590Fa3CAB3aC9e2168",
  )) as IUniswapV3Pool;
  contracts.PoolWbtcDai3000 = (await ethers.getContractAt(
    UniswapV3Pooljson.abi,
