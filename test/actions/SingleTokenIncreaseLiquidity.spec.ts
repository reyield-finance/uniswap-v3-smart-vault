...
describe("SingleTokenIncreaseLiquidity.sol", function () {
  ...
  describe("SingleTokenIncreaseLiquidityAction - singleTokenIncreaseLiquidity", function () {
    it("should single token increase liquidity with OP on OP/USDT pair", async function () {
      const singleIncrease = (await ethers.getContractAt(
        "ISingleTokenIncreaseLiquidity",
        positionManager.address,
      )) as ISingleTokenIncreaseLiquidity;
      ...
      const mintEvents: any = mintReceipt.events;
      const tokenId = mintEvents[mintEvents.length - 1].args.tokenId;

      await token0.connect(user).transfer(positionManager.address, amountIn);

      const txShare = await singleIncrease.connect(user).singleTokenIncreaseLiquidity({
        tokenId: tokenId,
      ...
      const tokenIdLog = events[events.length - 1].args.tokenId;
      const tokenInLog = events[events.length - 1].args.tokenIn;
      const amountInLog = events[events.length - 1].args.amountIn;
      const amount0Increased = events[events.length - 1].args.amount0Increased;
      const amount1Increased = events[events.length - 1].args.amount1Increased;
      ...
      expect(tokenIdLog).to.be.equal(tokenId);
      ...
      expect(swappedToken0.add(amount0Increased).add(amount0Leftover)).to.be.equal(amountIn);
      expect(amount1Increased.add(amount1Leftover)).to.be.equal(token1SwappedFromToken0);
    ...
    it("should single token increase liquidity with USDT on OP/USDT pair", async function () {
      const singleIncrease = (await ethers.getContractAt(
        "ISingleTokenIncreaseLiquidity",
        positionManager.address,
      )) as ISingleTokenIncreaseLiquidity;
      ...
      expect(count).to.be.equal(1);
      expect(swappedToken1.add(amount1Increased).add(amount1Leftover)).to.be.equal(amountIn);
      expect(amount0Increased.add(amount0Leftover)).to.be.equal(token0SwappedFromToken1);
