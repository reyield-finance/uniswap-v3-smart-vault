...
describe("ClosePositionOneShot.sol", function () {
  ...
  describe("ClosePositionOneShotAction - closePositionOneShot", function () {
    it("should close a uni position without return user", async function () {
      ...
      const close = (await ethers.getContractAt(
        "IClosePositionOneShot",
        positionManager.address,
      )) as IClosePositionOneShot;
      const txClosed = await close.connect(user).closePositionOneShot({ tokenId: tokenId, returnTokenToUser: false });
      const eventsClosed: any = (await txClosed.wait()).events;
      const tokenIdClosed = eventsClosed[eventsClosed.length - 1].args.tokenId;
      const amount0Collected = eventsClosed[eventsClosed.length - 1].args.amount0Collected as bigint;
      const amount1Collected = eventsClosed[eventsClosed.length - 1].args.amount1Collected as bigint;
      expect(tokenIdClosed).to.equal(tokenId);
      expect(await tokenUSDC.balanceOf(positionManager.address)).to.equal(amount0Collected);
      expect(await tokenWETH.balanceOf(positionManager.address)).to.equal(amount1Collected);
    });
    ...
    it("should close a uni position with return user", async function () {
      ...
      const txClosed = await close.connect(user).closePositionOneShot({ tokenId: tokenId, returnTokenToUser: true });
      const eventsClosed: any = (await txClosed.wait()).events;
      const tokenIdClosed = eventsClosed[eventsClosed.length - 1].args.tokenId;
      const amount0Collected = eventsClosed[eventsClosed.length - 1].args.amount0Collected;
      const amount1Collected = eventsClosed[eventsClosed.length - 1].args.amount1Collected;
      expect(tokenIdClosed).to.equal(tokenId);
      expect(tokenIdClosed).to.equal(tokenId);
      expect(await tokenUSDC.balanceOf(user.address)).to.equal(
        BigNumber.from(amount0Collected).add(usdcBalanceCurrent),
      );
      expect(await tokenWETH.balanceOf(user.address)).to.equal(
        BigNumber.from(amount1Collected).add(wethBalanceCurrent),
      );
    });
  });
});
